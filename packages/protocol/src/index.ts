...
/**
 * QR offer payload. Generated by the daemon, encoded in QR (base64url JSON),
 * scanned by the iOS client. Contains stable identity + sessionId + WS address.
 * Notably does NOT contain a one-time challenge — challenges are now generated
 * per-connection via clientNonce + serverNonce in the WS handshake.
 */
export const pairOfferFrame = z.object({
  type: z.literal("pair.offer"),
  v: z.number().int(),
  sessionId: z.string(), // pairing-attempt UUID
  ...
  daemonAddress: z.string(), // ws://host:port — fallback when mDNS unavailable
